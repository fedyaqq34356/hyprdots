...
Scope {
        ...
        for (const e of root.orbitEntries) {
            if (root.wifiTab) {
                ...
                out.push({
                    ...
                    detail: e.connected
                        ? (e.batteryAvailable
                            ? Math.round(e.battery * 100) + "%" : I18n.t("state.on"))
                        : (e.paired || e.bonded) ? I18n.t("net.pair") : I18n.t("net.new"),
                    glyph: Bt.icon(e),
                    bars: -1,
    ...
    readonly property string hubTitle: {
        if (root.wifiTab)
            return Network.connected ? Network.ssid
                 : Network.radio ? I18n.t("net.notConnected") : I18n.t("net.wifiOff");
        if (!Bt.present) return I18n.t("net.noAdapter");
        if (!Bt.powered) return I18n.t("net.btOff");
        return Bt.primary ? Bt.label(Bt.primary) : I18n.t("net.noConnections");
    }

    readonly property string hubSub: {
        if (root.wifiTab) {
            if (!Network.connected)
                return Network.radio ? Network.networks.length + I18n.t("net.networksNearby") : "";
            const parts = [Network.strength + "%"];
            if (Network.linkRate !== "") parts.push(Network.linkRate);
            return parts.join("  ·  ");
        }
        if (!Bt.powered) return "";
        if (Bt.connectedCount > 1) return Bt.connectedCount + I18n.t("net.devicesTwo");
        if (Bt.primary)
            return Bt.primary.batteryAvailable
                ? I18n.t("net.battery") + Math.round(Bt.primary.battery * 100) + "%"
                : I18n.t("net.connected");
        return Bt.devices.length + I18n.t("net.nearby");
    }
    ...
    readonly property string focusSub: {
        ...
        const bits = [];
        if (root.wifiTab) {
            bits.push(I18n.t("net.signal") + e.strength + "%");
            bits.push(e.secured ? I18n.t("net.secured") : I18n.t("net.open"));
            bits.push(e.active ? I18n.t("net.tapDisconnectSelf")
                               : e.secured ? I18n.t("net.tapAsk")
                                           : I18n.t("net.tapConnectSelf"));
        } else {
            if (e.connected) bits.push(I18n.t("net.connected"));
            else if (e.paired || e.bonded) bits.push(I18n.t("net.paired"));
            else bits.push(I18n.t("net.notPaired"));
            if (e.batteryAvailable)
                bits.push(I18n.t("net.battery") + Math.round(e.battery * 100) + "%");
            if (e.address) bits.push(String(e.address).toLowerCase());
            bits.push(e.connected ? I18n.t("net.tapDisconnect")
                                  : (e.paired || e.bonded) ? I18n.t("net.tapConnect")
                                                           : I18n.t("net.tapPair"));
        }
        return bits.join("  ·  ");
    }
                ...
                Item {
                    ...
                    Rectangle {
                        ...
                        Row {
                            ...
                            Text {
                                text: root.radioOn ? I18n.t("state.enabled") : I18n.t("state.disabled")
                    ...
                    Row {
                        ...
                        IconButton {
                            ...
                            tint: root.tint
                            mono: root.mono
                            tip: root.graphMode ? I18n.t("net.asList") : I18n.t("net.asGraph")
                        ...
                        Text {
                            ...
                            text: {
                                if (!root.wifiTab && !Bt.present)
                                    return I18n.t("net.noAdapterFound");
                                if (!root.radioOn)
                                    return root.wifiTab ? I18n.t("net.wifiOff")
                                                        : I18n.t("net.btOff");
                                if (!root.wifiTab && Bt.scanning) return I18n.t("net.scanning");
                                return root.wifiTab ? I18n.t("net.noNetworks") : I18n.t("net.noDevices");
                            }
                            color: Colors.fgDim
                            opacity: 0.5
                            font.family: root.mono
                    ...
                    Text {
                        ...
                        text: Network.error !== "" ? Network.error
                            ...
                            : root.hiddenCount > 0 && root.graphMode
                                ? "+" + root.hiddenCount + I18n.t("wall.inList")
                                : ""
