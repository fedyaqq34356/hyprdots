            ...
            Text {
                ...
                text: {
                    ...
                    if (row.wifi) {
                        ...
                        if (row.entry.secured) bits.push("защищена");
                        if (row.entry.active) bits.push("подключено");
                        return bits.join("  ·  ");
                    }
                    if (row.working)
                        return row.entry.pairing ? "сопряжение…"
                             : row.entry.state === BluetoothDeviceState.Connecting
                                ? "подключаю…" : "отключаю…";
                    const bits = [];
                    if (row.entry.connected) bits.push("подключено");
                    else if (row.entry.paired || row.entry.bonded) bits.push("сопряжено");
                    else bits.push("новое");
