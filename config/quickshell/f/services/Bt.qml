...
Singleton {
    ...
    function label(dev) {
        if (!dev) return "";
        return dev.name || dev.deviceName || dev.address || I18n.t("audio.device");
    }
    ...
    onPoweredChanged: {
        ...
        if (root.powered) Sfx.radioOn();
        else Sfx.radioOff();
    }

    onConnectedCountChanged: {
        if (!root.primed) return;
        if (root.connectedCount > root.lastConnectedCount) Sfx.netConnect();
        else if (root.connectedCount < root.lastConnectedCount) Sfx.netDisconnect();
        root.lastConnectedCount = root.connectedCount;
    }

    property int lastConnectedCount: 0
