...
Singleton {
    ...
    function toggle()   { if (canToggle) { Sfx.press(); player.togglePlaying(); } }
    function next()     { if (canNext)   { Sfx.swoosh(); player.next(); } }
    function previous() { if (canPrev)   { Sfx.swoosh(); player.previous(); } }
